...
if ($concepto_id <= 0 || empty($fecha_desde)) {
...
switch ($forma_calculo) {
    case 'TABLA_CATEGORIA':
        ...
        $porcentaje = 0;
        $escalafon_id = null; // si no lo usás para este tipo, lo limpiamos
        break;
...
/*
|--------------------------------------------------------------------------
| Evitar duplicados exactos de vigencia para el mismo criterio
|--------------------------------------------------------------------------
| Para TABLA_CATEGORIA controla por concepto + categoría + fecha_desde
| Para otros, controla por concepto + categoría/escalafón + fecha_desde
*/
...
$stmt = $conexion->prepare("
    ...
    (concepto_id, categoria_id, escalafon_id, monto, porcentaje, fecha_desde, fecha_hasta, activo)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
");
