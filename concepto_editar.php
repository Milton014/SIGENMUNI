...
<form action="concepto_actualizar.php" method="POST" id="formConcepto" novalidate>
...
<input 
    ...
    name="codigo"
    id="codigo"
    value="<?php echo htmlspecialchars($concepto['codigo']); ?>"
>
</div>
...
<option value="REMUNERATIVO" <?php if ($concepto['categoria'] === 'REMUNERATIVO') echo 'selected'; ?>>
REMUNERATIVO
</option>

<option value="NO_REMUNERATIVO" <?php if ($concepto['categoria'] === 'NO_REMUNERATIVO') echo 'selected'; ?>>
NO_REMUNERATIVO
</option>

<option value="ASIGNACION_FAMILIAR" <?php if ($concepto['categoria'] === 'ASIGNACION_FAMILIAR') echo 'selected'; ?>>
ASIGNACION_FAMILIAR
</option>

<option value="DESCUENTO" <?php if ($concepto['categoria'] === 'DESCUENTO') echo 'selected'; ?>>
DESCUENTO
</option>

<option value="APORTE_PATRONAL" <?php if ($concepto['categoria'] === 'APORTE_PATRONAL') echo 'selected'; ?>>
APORTE_PATRONAL
</option>
...
<option value="FIJO" <?php if ($concepto['forma_calculo'] === 'FIJO') echo 'selected'; ?>>
FIJO
</option>

<option value="TABLA_CATEGORIA" <?php if ($concepto['forma_calculo'] === 'TABLA_CATEGORIA') echo 'selected'; ?>>
TABLA POR CATEGORÍA
</option>

<option value="PORCENTAJE" <?php if ($concepto['forma_calculo'] === 'PORCENTAJE') echo 'selected'; ?>>
PORCENTAJE
</option>

<option value="MANUAL" <?php if ($concepto['forma_calculo'] === 'MANUAL') echo 'selected'; ?>>
MANUAL
</option>

<option value="FORMULA" <?php if ($concepto['forma_calculo'] === 'FORMULA') echo 'selected'; ?>>
FORMULA
</option>
...
<input 
    ...
    name="porcentaje"
    id="porcentaje"
    value="<?php echo htmlspecialchars($concepto['porcentaje']); ?>"
>
</div>
...
<input 
    ...
    name="monto_fijo"
    id="monto_fijo"
    value="<?php echo htmlspecialchars($concepto['monto_fijo']); ?>"
>
...
<input 
    ...
    id="base_calculo"
    maxlength="50"
    value="<?php echo htmlspecialchars($concepto['base_calculo'] ?? ''); ?>"
>
</div>
...
<input 
    ...
    id="orden_calculo"
    min="0"
    value="<?php echo htmlspecialchars($concepto['orden_calculo']); ?>"
>
</div>
...
<input 
    ...
    name="fecha_desde"
    id="fecha_desde"
    value="<?php echo htmlspecialchars($concepto['fecha_desde'] ?? ''); ?>"
>
</div>
...
<input 
    ...
    name="fecha_hasta"
    id="fecha_hasta"
    value="<?php echo htmlspecialchars($concepto['fecha_hasta'] ?? ''); ?>"
>
</div>
...
<textarea 
    name="descripcion"
    id="descripcion"
><?php echo htmlspecialchars($concepto['descripcion'] ?? ''); ?></textarea>
...
<?php if ((int)$concepto['requiere_manual'] === 1) echo 'checked'; ?>>
Requiere Manual
</label>
</div>
...
<?php if ((int)$concepto['aplica_sac'] === 1) echo 'checked'; ?>>
Aplica SAC
</label>
</div>
...
<div class="check-item">
<label>
<input type="checkbox" name="visible_recibo" value="1"
<?php if ((int)$concepto['visible_recibo'] === 1) echo 'checked'; ?>>
Visible en Recibo
</label>
</div>
...
<?php if ((int)$concepto['activo'] === 1) echo 'checked'; ?>>
Activo
</label>
</div>
...
document.getElementById("formConcepto").addEventListener("submit", function(e){
    ...
    const monto = document.getElementById("monto_fijo").value.trim();

    if(codigo === ""){
        mostrarError("Debe ingresar el código.","codigo");
        return;
    }
    ...
    if(forma === "FIJO"){
        ...
        if(monto === "" || parseFloat(monto) <= 0){
            mostrarError(
                "Debe ingresar un monto fijo mayor a cero.",
                "monto_fijo"
            );
            return;
        }
